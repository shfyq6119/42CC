...
/*   Updated: 2024/01/02 15:49:51 by mm-isa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include <unistd.h>
int	is_alpha(int c)
...
int	main(int ac, char **av)					//however many arguments, str_capitalize
{
	...
	if (ac > 1)
	{
		while (i < ac) // master loop condition = not final argument, continue
		{
			...
			while (av[i][++j])				//start moving on to next letter at the start of loop
			{	
				if (is_alpha(av[i][j]) == 2)						// if upper
					av[i][j] += 32;									// to lower
				if (is_alpha(av[i][j]) == 1 && (av[i][j-1] == 32	// if lower AND before is ' ' or '\t'
					|| av[i][j-1] == 9)) 	
					av[i][j] -= 32;									// to_upper
